...
function AtD_restore_text_area()
{
	...
	jQuery('#content').val( content.replace(/\&lt\;/g, '<').replace(/\&gt\;/, '>').replace(/\&amp;/g, '&') );
...
function AtD_check(callback) {
	...
	if (jQuery('#ed_AtD').val() == AtD.getLang('button_edit_text', 'edit text')) {                               
		...
		/* replace the div */
		if (navigator.appName == 'Microsoft Internet Explorer') {
			jQuery('#content').replaceWith( '<div class="input" id="content">' + jQuery('#content').val().replace(/\&/g, '&amp;').replace(/[\n\r\f]/gm, '<BR class="atd_remove_me">') + '</div>' );
			jQuery('#content').css( { 'overflow' : 'auto', 'background-color' : 'white', 'color' : 'black' } );
		} else {
			jQuery('#content').replaceWith( '<div class="input" id="content">' + jQuery('#content').val().replace(/\&/g, '&amp;') + '</div>' );
